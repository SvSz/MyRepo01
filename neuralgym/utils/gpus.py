...
def set_gpus(gpus):
    ...
    if not isinstance(gpus, list):
        gpus = [gpus]
    os.environ['CUDA_VISIBLE_DEVICES'] = '0'.join(str(e) for e in gpus)           #### changed from = ',' to ='0' if only 1 GPU
...
def get_gpus(num_gpus=1, dedicated=True, verbose=True):
    ...
    if ret == []:
        # error, get no GPU
        os.environ['CUDA_VISIBLE_DEVICES'] = '0'                                     #### changed from = '' to ='0' if only 1 GPU
